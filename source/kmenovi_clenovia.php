...
if (!empty($_POST['id']) && !empty($_POST['obsah']) && !empty($_POST['stlpec'])) {
    $id = $_POST['id'];
    $obsah = $_POST['obsah'];
    $stlpec = $_POST['stlpec'];
...
?>
<form method="post">
    <input type="submit" name="export" value="Exportuj zoznam">
</form>
<?php paticka(); ?>
</html>
...
{ ?>
    <div>
    <br>
    <table style="width:100%;" class="tabulkaVykonou tablesorter" id="kmenovi">
        <thead>
        <tr>
            <th class="prvy header"></th>
            <th class="prvy header">Meno</th>
            <th class="prvy header">Priezisko</th>
            <th class="prvy header">Pohlavie</th>
            <th class="prvy header">Dátum narodenia</th>
            <th class="prvy header">Krajina narodenia</th>
            <th class="prvy header">Štátna príslušnosť</th>
            <th class="prvy header">Krajina trvalého bydliska</th>
            <th class="prvy header">Ulica</th>
            <th class="prvy header">Číslo domu</th>
            <th class="prvy header">PSČ</th>
            <th class="prvy header">Mesto</th>
            <th class="prvy header">Telefón</th>
            <th class="prvy header">Mail</th>
            <th class="prvy header">Číslo čipu</th>
            <th class="prvy header">Registračné číslo</th>
            <th class="prvy header"></th>
        </tr>
        </thead>
        <?php
        $db = napoj_db();
        $sql = <<<EOF
         SELECT * from Kmenovi_clenovia AS k JOIN Pouzivatelia AS p ON p.id_kmen_clen = k.id ORDER BY p.priezvisko;
EOF;
        $ret = $db->query($sql);
...
<script>
    $(document).ready(function () {
        $("#kmenovi").tablesorter({dateFormat: "uk"});
    });
</script>
